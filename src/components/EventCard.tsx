...
export default function EventCard({
  ...
  const getCalendarUrl = (event: Event) => {
    const startDate = new Date(
      `${event.date}T${event.time.split("-")[0]}-03:00`,
    );
  ...
  return (
    <article
      className={`rounded-lg shadow-xs border overflow-hidden transition-all hover:shadow-lg ${
        featured ? "flex flex-col lg:flex-row" : "flex flex-col h-full"
      } ${
        isToday
      ...
      <div className="flex-1 flex flex-col">
        <div className="p-6 flex-grow flex flex-col">
          <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-3">
            <time dateTime={`${event.date}T${event.time.split("-")[0]}-03:00`}>
              {new Date(
                `${event.date}T${event.time.split("-")[0]}-03:00`,
